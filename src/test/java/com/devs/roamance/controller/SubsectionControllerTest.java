...
class SubsectionControllerTest {
  ...
  @MockBean private SubsectionService subsectionService;
  ...
  void getAllSubsectionsShouldReturnSubsectionsList() throws Exception {
    ...
    when(subsectionService.getAll(anyInt(), anyInt(), anyString(), anyString()))
        .thenReturn(responseDto);

    // When & Then
    mockMvc
        .perform(
            get("/travel/subsections")
                .param("pageNumber", "0")
                .param("pageSize", "10")
                .param("sortBy", "id")
                .param("sortDir", "asc"))
  ...
  void getSubsectionByIdShouldReturnSubsection() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/travel/subsections/{id}", subsectionId))
        .andDo(print())
        .andExpect(status().isOk());
  }
  ...
  void createSubsectionShouldCreateWithValidRequest() throws Exception {
    ...
    mockMvc
        .perform(
            post("/travel/subsections").contentType(MediaType.APPLICATION_JSON).content(content))
        .andDo(print())
        .andExpect(status().isOk());
  }
  ...
  void updateSubsectionShouldUpdateWithValidRequest() throws Exception {
    ...
    when(subsectionService.update(any(SubsectionUpdateRequestDto.class), any(UUID.class)))
        .thenReturn(responseDto);
    ...
    mockMvc
        .perform(
            put("/travel/subsections/{id}", subsectionId)
  ...
  void deleteSubsectionShouldDeleteWhenValidIdIsProvided() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(delete("/travel/subsections/{id}", subsectionId))
        .andDo(print())
        .andExpect(status().isOk());
  }
}
