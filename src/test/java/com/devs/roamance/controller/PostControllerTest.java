...
class PostControllerTest {
    ...
    void getPostsByIdsShouldReturnPostsList() throws Exception {
        ...
        mockMvc.perform(post("/posts/by-ids")
                .contentType(MediaType.APPLICATION_JSON)
                .content(content)
                .param("pageNumber", "0")
                .param("pageSize", "10"))
    ...
    void getPostsByUserIdShouldReturnPostsList() throws Exception {
        ...
        when(postService.getByUserId(any(UUID.class), anyInt(), anyInt(), anyString(), anyString())).thenReturn(responseDto);

        // When & Then
        mockMvc.perform(get("/posts/by-user/{userId}", userId)
                .param("pageNumber", "0")
                .param("pageSize", "10")
                .param("sortBy", "audit.createdAt")
                .param("sortDir", "desc"))
    ...
    void getSavedPostsByCurrentUserShouldReturnPostsList() throws Exception {
        ...
        when(postService.getSavedByCurrentUser(anyInt(), anyInt(), anyString(), anyString())).thenReturn(responseDto);
    ...
    void getUsersWhoLikedPostShouldReturnUsersList() throws Exception {
        ...
        when(postService.getUsersWhoLiked(any(UUID.class), anyInt(), anyInt(), anyString(), anyString()))
                .thenReturn(responseDto);

        // When & Then
        mockMvc.perform(get("/posts/liked-by/{postId}", postId)
                .param("pageNumber", "0")
                .param("pageSize", "10")
                .param("sortBy", "id")
                .param("sortDir", "asc"))
    ...
    void updatePostShouldUpdateWithValidRequest() throws Exception {
        ...
        // When & Then
        String content = objectMapper.writeValueAsString(requestDto);
        mockMvc.perform(put("/posts/update/{postId}", postId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(content))
                .andDo(print())
                .andExpect(status().isOk());
    ...
    void togglePostSaveShouldToggleWhenRequested() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/posts/save/{postId}", postId))
                .andDo(print())
                .andExpect(status().isOk());
    }
    ...
    void deletePostShouldDeleteWhenValidIdIsProvided() throws Exception {
        ...
        // When & Then
        mockMvc.perform(delete("/posts/delete/{postId}", postId))
                .andDo(print())
                .andExpect(status().isOk());
    }
}