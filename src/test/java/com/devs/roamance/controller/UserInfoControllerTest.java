...
class UserInfoControllerTest {
  ...
  void getAllUsersInfoShouldReturnUserInfoList() throws Exception {
    ...
    when(userInfoService.getAll(anyInt(), anyInt(), anyString(), anyString()))
  ...
  void getUserInfoByIdShouldReturnUserInfo() throws Exception {
    ...
    // When & Then
    mockMvc.perform(get("/users/info/{id}", id)).andDo(print()).andExpect(status().isOk());
  }
  ...
  void getUserProfileShouldReturnCurrentUserInfo() throws Exception {
    ...
    // When & Then
    mockMvc.perform(get("/users/info/me")).andDo(print()).andExpect(status().isOk());
  }
  ...
  void createUserInfoShouldCreateWithValidRequest() throws Exception {
    ...
    mockMvc
        .perform(post("/users/info").contentType(MediaType.APPLICATION_JSON).content(content))
        .andDo(print())
  ...
  void updateUserInfoShouldUpdateWithValidRequest() throws Exception {
    ...
    when(userUtil.getAuthenticatedUser()).thenReturn(mockUser);
    when(userInfoService.updateByUserId(any(UserInfoRequestDto.class), any(UUID.class)))
        .thenReturn(responseDto);

    // When & Then
    String content = objectMapper.writeValueAsString(requestDto);
    mockMvc
        .perform(put("/users/info").contentType(MediaType.APPLICATION_JSON).content(content))
        .andDo(print())
        .andExpect(status().isOk());
  }
  ...
  void updateUserInfoByIdShouldUpdateWithValidRequest() throws Exception {
    ...
    when(userInfoService.update(any(UserInfoRequestDto.class), any(UUID.class)))
        .thenReturn(responseDto);
    ...
    mockMvc
        .perform(
            put("/users/info/{id}", id).contentType(MediaType.APPLICATION_JSON).content(content))
        .andDo(print())
        .andExpect(status().isOk());
  }
  ...
  void deleteUserInfoShouldDeleteCurrentUserInfo() throws Exception {
    ...
    // When & Then
    mockMvc.perform(delete("/users/info")).andDo(print()).andExpect(status().isOk());
  }
  ...
  void deleteUserInfoByIdShouldDeleteById() throws Exception {
    ...
    // When & Then
    mockMvc.perform(delete("/users/info/{id}", id)).andDo(print()).andExpect(status().isOk());
  }
}
