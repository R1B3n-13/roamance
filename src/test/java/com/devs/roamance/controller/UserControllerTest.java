...
class UserControllerTest {
  ...
  void getUserByEmailShouldReturnUser() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/users/by-email").param("email", email))
        .andDo(print())
        .andExpect(status().isOk());
  }
  ...
  void updateUserShouldUpdateWithValidRequest() throws Exception {
    ...
    when(userUtil.getAuthenticatedUser()).thenReturn(mockUser);
    when(userService.update(any(UserUpdateRequestDto.class), any(UUID.class)))
        .thenReturn(responseDto);

    // When & Then
    String content = objectMapper.writeValueAsString(requestDto);
    mockMvc
        .perform(put("/users").contentType(MediaType.APPLICATION_JSON).content(content))
        .andDo(print())
        .andExpect(status().isOk());
  }
  ...
  void deleteUserShouldDeleteWhenRequested() throws Exception {
    ...
    // When & Then
    mockMvc.perform(delete("/users")).andDo(print()).andExpect(status().isOk());
  }
}
