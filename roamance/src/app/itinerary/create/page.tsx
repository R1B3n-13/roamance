...
export default function CreateItineraryPage() {
  ...
  const [withDayPlans, setWithDayPlans] = useState(false);
  ...
  const [newRouteLocation, setNewRouteLocation] = useState<{ [key: number]: Location }>({});
  ...
  const updateActivityField = (dayIndex: number, field: keyof ActivityCreateRequest, value: string | number) => {
  ...
  const handleRouteLocationChange = (dayIndex: number, lat: number, lng: number) => {
    setNewRouteLocation(prev => ({
      ...prev,
  ...
  const addRouteLocation = (dayIndex: number) => {
    ...
    const updatedLocations = [...(routePlanLocations[dayIndex] || []), newRouteLocation[dayIndex]];
  ...
  const removeRouteLocation = (dayIndex: number, locationIndex: number) => {
    const updatedLocations = (routePlanLocations[dayIndex] || [])
  ...
  useEffect(() => {
    if (withDayPlans && startDate && endDate) {
  ...
  }, [startDate, endDate, withDayPlans]);
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      if (withDayPlans && dayPlans.length > 0) {
        ...
        response = await ItineraryService.createItineraryWithDetails(itineraryWithDetails);
  ...
  // Handler for toggling day plans creation
  const handleToggleDayPlans = (value: boolean) => {
    setWithDayPlans(value);
    form.setValue('create_day_plans', value);
  };

  // Handler for adding a note to a specific day plan
  const handleAddDayPlanNote = (dayIndex: number, note: string) => {
    if (note.trim()) {
      const updatedDayPlans = [...dayPlans];
      updatedDayPlans[dayIndex].notes = [...updatedDayPlans[dayIndex].notes, note.trim()];
      setDayPlans(updatedDayPlans);
    }
  };

  // Handler for removing a note from a specific day plan
  const handleRemoveDayPlanNote = (dayIndex: number, noteIndex: number) => {
    const updatedDayPlans = [...dayPlans];
    updatedDayPlans[dayIndex].notes = updatedDayPlans[dayIndex].notes.filter(
      (_, i) => i !== noteIndex
    );
    setDayPlans(updatedDayPlans);
  };
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      checked={withDayPlans}
                      onChange={(e) => handleToggleDayPlans(e.target.checked)}
                    />
                    <label
                  ...
                  {withDayPlans && dayPlans.length > 0 && (
                    <div className="mt-4 space-y-6">
                      <p className="text-sm text-muted-foreground">
                        You&apos;re creating {dayPlans.length} day plans, one for each day of your trip.
                        You can add specific notes for each day.
                      </p>

                      {dayPlans.map((dayPlan, dayIndex) => (
                        <div key={dayIndex} className="p-4 border border-muted rounded-lg space-y-3">
                          <h3 className="text-sm font-medium">
                            Day {dayIndex + 1}: {format(dayPlan.date, 'EEEE, MMMM d, yyyy')}
                          </h3>
                          ...
                          <div className="mt-4">
                            ...
                            >
                              {showActivityForm[dayIndex] ? 'Hide Activities' : 'Add Activity'}
                            </Button>
                              ...
                              <div className="mt-2 p-3 border border-muted/30 rounded-lg space-y-4">
                                ...
                                <div className="space-y-1">
                                  ...
                                  <select
                                    ...
                                    value={currentActivityInputs[dayIndex]?.type || ActivityType.SIGHTSEEING}
                                    onChange={(e) => updateActivityField(dayIndex, 'type', e.target.value)}
                                  >
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  <div className="space-y-1">
                                    ...
                                    <Input
                                      type="time"
                                      className="bg-muted/50 text-xs"
                                      value={currentActivityInputs[dayIndex]?.start_time || '09:00'}
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <Input
                                      type="time"
                                      className="bg-muted/50 text-xs"
                                      value={currentActivityInputs[dayIndex]?.end_time || '10:00'}
                                ...
                                <div className="space-y-1">
                                  ...
                                  <div className="h-48 border border-muted/30 rounded-lg overflow-hidden">
                                    <LocationPickerMap
                                      ...
                                      onLocationChangeAction={(lat, lng) => {
                                        const updated = { ...currentActivityInputs[dayIndex] };
                                        updated.location = { latitude: lat, longitude: lng };
                                        setCurrentActivityInputs(prev => ({
                                          ...prev,
                                          [dayIndex]: updated
                                        }));
                                  ...
                                  <p className="text-xs text-muted-foreground mt-1">
                                    Click on the map to select a location for this activity
                                  </p>
                                </div>
                                ...
                                <div className="space-y-1">
                                  ...
                                  <div className="relative">
                                    ...
                                    <Input
                                      ...
                                      min="0"
                                      step="0.01"
                                      value={currentActivityInputs[dayIndex]?.cost || 0}
                            ...
                            {dayPlan.activities.length > 0 && (
                              <div className="mt-4 space-y-1.5">
                                <h4 className="text-xs font-medium">Activities ({dayPlan.activities.length})</h4>
                          ...
                          <div className="mt-4">
                            ...
                            >
                              {showRoutePlan[dayIndex] ? 'Hide Route Plan' : 'Add Route Plan'}
                            </Button>
                            ...
                            {showRoutePlan[dayIndex] && (
                              <div className="mt-2 space-y-4 rounded-lg border border-border p-4">
                                {/* Route Description */}
                                <div className="space-y-2">
                                  <label className="text-xs font-medium">Route Description</label>
                                  <Textarea
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                  <div className="space-y-2">
                                    ...
                                    <Input
                                      ...
                                      step="0.1"
                                      placeholder="0.0"
                                      value={routePlanTotalDistances[dayIndex] || '0'}
                                      onChange={(e) => updateTotalDistance(dayIndex, e.target.value)}
                                      className="bg-muted/50 text-xs"
                                    />
                                  </div>
                                  <div className="space-y-2">
                                    <label className="text-xs font-medium">Total Time (minutes)</label>
                                    <Input
                                      type="number"
                                      min="0"
                                      step="1"
                                      placeholder="0"
                                      value={routePlanTotalTimes[dayIndex] || '0'}
                                ...
                                <div className="space-y-3">
                                  <label className="text-xs font-medium">Route Locations</label>
                                    ...
                                    <div className="mt-3 flex justify-between">
                                      <p className="text-xs text-muted-foreground">
                                        {newRouteLocation[dayIndex] &&
                                         (newRouteLocation[dayIndex].latitude !== 0 || newRouteLocation[dayIndex].longitude !== 0) ? (
                                          <span>Selected: {newRouteLocation[dayIndex].latitude.toFixed(6)}, {newRouteLocation[dayIndex].longitude.toFixed(6)}</span>
                                        ) : (
                                      ...
                                      <Button
                                        ...
                                        disabled={!newRouteLocation[dayIndex] ||
                                                (newRouteLocation[dayIndex].latitude === 0 &&
                                                 newRouteLocation[dayIndex].longitude === 0)}
                                  ...
                                  {(routePlanLocations[dayIndex]?.length > 0) ? (
                                    <div className="mt-2">
                                      <h4 className="text-xs font-medium mb-2">Added Locations ({routePlanLocations[dayIndex].length})</h4>
