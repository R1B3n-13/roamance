...
import { Activity, ArrowLeft, Calendar, Calendar as CalendarIcon, CheckCircle2, ChevronDown, Clock, Eye, Map, MapPin, Route, Star, X } from 'lucide-react';
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="bg-background rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col border border-muted/30"
          >
            ...
            <div className="relative">
              {coverImage ? (
                <div className="relative h-56 md:h-72 lg:h-80 overflow-hidden">
                  ...
                  <button
                    onClick={onClose}
                    className="absolute top-4 right-4 p-2 rounded-full bg-black/30 text-white hover:bg-black/50 z-20 transition-all duration-300 backdrop-blur-sm hover:rotate-90"
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 z-20">
                    ...
                    >
                      <Badge className="bg-white/20 text-white backdrop-blur-sm border-none px-3 py-1">
                        {subsectionCount} {subsectionCount === 1 ? 'Section' : 'Sections'}
                      </Badge>
                ...
                <div className="bg-gradient-to-br from-indigo-600 via-purple-600 to-violet-600 p-8 relative">
                  <button
                    onClick={onClose}
                    className="absolute top-4 right-4 p-2 rounded-full bg-white/20 text-white hover:bg-white/30 z-20 transition-all duration-300 backdrop-blur-sm hover:rotate-90"
                    aria-label="Close journal view"
                  >
                    <X className="w-5 h-5" />
                  </button>
              ...
              <div className="px-6 md:px-8 py-6">
                ...
                <div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center p-4 rounded-xl bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-800/20">
                    ...
                    <div>
                      <div className="text-xs text-indigo-500 dark:text-indigo-400 font-medium uppercase tracking-wider mb-1">Location</div>
                  ...
                  <div className="flex items-center p-4 rounded-xl bg-purple-50 dark:bg-purple-900/10 border border-purple-100 dark:border-purple-800/20">
                    ...
                    <div>
                      <div className="text-xs text-purple-500 dark:text-purple-400 font-medium uppercase tracking-wider mb-1">Created</div>
                      <div className="text-sm text-foreground">
                  ...
                  <div className="mb-8">
                    <Button
                      ...
                      className={cn(
                        ...
                        showMap && "border-indigo-400/50 bg-indigo-50/50 dark:bg-indigo-900/10"
                      )}
                    >
                      <span className="flex items-center text-foreground">
                        <Map className="w-5 h-5 mr-2 text-indigo-500 group-hover:text-indigo-600 transition-colors duration-200" />
                        <span className="font-medium">{showMap ? 'Hide Map Location' : 'Show Map Location'}</span>
                      </span>
                      <div className={cn(
                        "w-6 h-6 rounded-full flex items-center justify-center transition-all duration-300",
                        showMap ? "bg-indigo-100 dark:bg-indigo-900/30 rotate-180" : "bg-muted/50"
                      )}>
                ...
                <div>
                  <div className="flex items-center mb-6">
                    <h3 className="text-xl font-semibold text-foreground mr-3">Journal Sections</h3>
                    <Badge variant="outline" className="bg-indigo-50 text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-400 border-indigo-200 dark:border-indigo-800/30">
                      {subsectionCount} {subsectionCount === 1 ? 'Section' : 'Sections'}
                    </Badge>
                  </div>
                  ...
                  {journal.subsections && journal.subsections.length > 0 ? (
                          ...
                          >
                            ...
                            >
                              <div className="flex items-center">
                                <div className={`flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center bg-gradient-to-br ${getTypeColor(subsection.type)} shadow-sm`}>
                                  {getSubsectionIcon(subsection.type)}
                                </div>
                                <div className="ml-4">
                                  <h4 className="font-medium text-foreground">{subsection.title}</h4>
                                  <div className="flex items-center text-xs text-muted-foreground mt-1 space-x-2">
                                    <Badge variant="secondary" className={colors.badge}>{subsection.type}</Badge>
                                  ...
                                  <div className={cn("p-5", colors.bg)}>
                                          ...
                                          <div>
                                            ...
                                            <div className={cn(
                                              ...
                                              {(subsection as any).locations.map((location: any, index: number) => (
                                                <div key={index} className="flex items-center text-sm relative pl-6">
                                                  <div className={cn(
                                                    "absolute -left-[17px] w-8 h-8 rounded-full flex items-center justify-center text-white z-10",
                                                    index === 0 ? "bg-green-500" :
                                                    index === (subsection as any).locations.length - 1 ? "bg-red-500" :
                                                  ...
                                                  <div className="bg-background/80 backdrop-blur-sm rounded-lg py-2 px-3 border border-muted/30 w-full">
                                                    Stop {index + 1}: {location.latitude.toFixed(4)}, {location.longitude.toFixed(4)}
                                                  </div>
                                                </div>
                                              ))}
                                            </div>
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                      ...
                                      {subsection.checklists && subsection.checklists.length > 0 && (
                                        <div>
                                          <h5 className="text-sm font-medium mb-3 flex items-center">
                                            <CheckCircle2 className={cn("w-4 h-4 mr-2", colors.icon)} />
                                            Checklist
                                            <Badge className={cn("ml-2 text-xs", colors.badge)}>
                                          ...
                                          <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 scrollbar-hide">
                                            {subsection.checklists.map((item, i) => (
                                              <div key={i} className="flex items-center p-3 rounded-lg bg-background/80 backdrop-blur-sm border border-muted/30 text-sm text-foreground">
                                                <div className={cn(
                    ...
                    >
                      ...
                      <h4 className="text-lg font-medium text-foreground mb-2">No sections yet</h4>
